...
import com.paysave.databinding.FragmentELoadBinding;
...
public class PaybillsFragment extends com.paysave.RequireLoginFragment {
        ...
        binding.payButton.setOnClickListener(v -> {
            if(isContactNumberValid()){
                if(selectedSIM == null || selectedSIM.isEmpty()){
                    Toast.makeText(requireContext(), "Please select a valid Account Number", Toast.LENGTH_LONG).show();
                    return;
                }
                showConfirmationDialog();
            }
            else{
                Toast.makeText(requireContext(), "Invalid Account number!", Toast.LENGTH_LONG).show();
            }
        });
    ...
    private void checkIfHasEnoughBalance() {
        ...
        if(receiver == null){
            binding.editTextPhoneRecipient.setError("Account Number Invalid");
            return;
        }
    ...
    private void showConfirmationDialog() {
        ...
        builder.setTitle("Confirmation")
                .setMessage(String.format("Are you sure the Account number is correct?\n Amount of %s will be paid %s?", getFormattedAmount(value), receiver))
                    ...
                    public void onComplete(@NonNull Task<QuerySnapshot> task) {
                        if(task.isSuccessful()){
                            if(task.getResult().isEmpty()) {
                                binding.editTextPhoneRecipient.setError("Invalid Account Number");
                                return;
                            }
                            if(getAmount() > 0) {
